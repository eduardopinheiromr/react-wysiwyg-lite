...
import { Separator, createButton } from "./button";
...
const Wrapper = ({
	...
	dictionary?: React.ComponentProps<typeof EditorProvider>["dictionary"];
}) => {
	const providerProps = dictionary !== undefined ? { dictionary } : {};
	return <EditorProvider {...providerProps}>{children}</EditorProvider>;
};
...
describe("createButton", () => {
	...
	it("has rsw-btn class", () => {
		...
		expect(
			container.querySelector("button")?.classList.contains("rsw-btn"),
		).toBe(true);
	});
	...
	it("uses the dictionary title when a dictionaryKey is provided", () => {
		const BtnTest = createButton("Bold", "B", "bold", {
			dictionaryKey: "bold",
		});
		render(
			<Wrapper dictionary={ptBR}>
				<BtnTest />
			</Wrapper>,
		);
	...
	it("merges partial dictionary overrides with the default dictionary", () => {
		const BtnTest = createButton("Bold", "B", "bold", {
			dictionaryKey: "bold",
		});
		render(
			<Wrapper dictionary={{ toolbar: { bold: "Custom bold" } }}>
				<BtnTest />
			</Wrapper>,
		);
		expect(screen.getByTitle("Custom bold")).not.toBeNull();
	});
});
